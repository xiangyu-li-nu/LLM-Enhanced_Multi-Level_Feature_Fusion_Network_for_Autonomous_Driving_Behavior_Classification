...
# 10. Define multimodal model (remove multi-scale convolution)
class MultiModalClassifier(nn.Module):
    def __init__(self, num_numerical_features, num_labels, roberta_model_name='roberta-base'):
        ...
        # Numerical features branch: single-scale 1D convolution + spatio-temporal attention mechanism + deeper convolution network
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=64, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
        self.pool = nn.AdaptiveMaxPool1d(1)

        # Spatio-temporal attention mechanism
        self.spatio_temporal_attention = SpatioTemporalAttention(in_channels=64, reduction=16, kernel_size=7)
        ...
        # Fully connected layer for processing convolution features
    ...
    def forward(self, input_ids, attention_mask, numerical):
        ...
        # No longer concatenate multi-scale features
        combined_conv = conv1_out  # [batch_size, 64, num_features]

        # Spatio-temporal attention mechanism
        combined_conv = self.spatio_temporal_attention(combined_conv)  # [batch_size, 64, num_features]
...
torch.save(model.state_dict(), 'multi_modal_roberta_classifier_no_multiscale.pth')
print("Model saved as 'multi_modal_roberta_classifier_no_multiscale.pth'")
